---
title: Vercel Agent Skills
---

# Vercel Agent Skills

## What This Page Is For

Show where Vercel-origin skills fit in this handbook: as narrow, high-value augmentations for React, UI, and framework-specific checks.

## When To Use This

Use this page when the task is already in motion and you want targeted guidance instead of a full workflow rewrite.

## If You Only Remember One Thing

Vercel skills sharpen one decision at a time. They complement `superpowers`; they do not replace the broader process layer.

## Where They Fit

Think of the stack this way:

- `superpowers` decides how the work should be run
- Vercel skills decide how to inspect or improve a specific React or UI concern
- Agent Skills and `skills.sh` decide how reusable instructions are packaged and shared

That makes Vercel skills useful when the question is narrow: is this component rendering efficiently, is this UI pattern sound, or does this page need a framework-specific guardrail?

## Practical Uses

- `vercel-react-best-practices` for React rendering, data flow, and performance checks
- framework-specific review when you want a targeted second opinion without switching into a full process model

In Codex-first use, that usually means invoking the targeted skill inside a broader workflow that already decided the scope and verification steps.

## What This Is Not

- not a full methodology for planning and execution
- not a replacement for `brainstorming`, `writing-plans`, or verification
- not a license to turn one skill into a general-purpose agent workflow

## How It Fits With Superpowers

- use `superpowers` to control the overall process
- use Vercel skills when the task narrows to a framework-specific implementation concern
- keep the prompt pointed at the exact issue you want reviewed, then return to the broader workflow for planning and verification

## Source References

- [vercel-labs/agent-skills](https://github.com/vercel-labs/agent-skills)
